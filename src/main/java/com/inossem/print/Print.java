...
public class Print {
	...
	public interface TSCLIB_DLL_GP3150TN extends Library {
		String path = Print.class.getResource("/").getPath().substring(1)
				+ "TSCLIB";
		TSCLIB_DLL_GP3150TN INSTANCE = (TSCLIB_DLL_GP3150TN) Native
				.loadLibrary(path, TSCLIB_DLL_GP3150TN.class);
...
//		sb.append("690,");// Y QRCODE條碼左上角Y座標
	...
	public static void Print_Qrcode(String barCode, String spcode,
			String order, String line, String des,String supplier,String fourPos,String speStore,String machineName, int index) {
		TSCLIB_DLL_GP3150TN.INSTANCE.openport(machineName);// 打开 打印机
																		// 端口.
		TSCLIB_DLL_GP3150TN.INSTANCE
				.setup("60", "90", "3", "10", "0", "3", "0");
		...
		TSCLIB_DLL_GP3150TN.INSTANCE
				.sendcommand(CMD_QRCODE_FROMT_GP3150TN(barCode,index));
	...
	private static void renderTxt(String txt, int fx, int y, int fontstyle) {
		TSCLIB_DLL_GP3150TN.INSTANCE.windowsfont(fx, y, FONT_HEIGHT, 90, fontstyle, 0,
				"simsun", txt);
	}
	...
	private static int renderMDesc(String des, int fontNum, int x, int y) {
		if (des.length() > fontNum) {
			for (int i = 0; i <= (des.length() / fontNum); i++) {
				if (i == 0) {
					TSCLIB_DLL_GP3150TN.INSTANCE.windowsfont(x, y, FONT_HEIGHT, 90, 0,
							0, "simsun",  des.substring(0, fontNum));
				} else {
					x = x + 40;
					int y2 = y;
					if ((i + 1) * fontNum <= des.length()) {
						TSCLIB_DLL_GP3150TN.INSTANCE.windowsfont(x, y2, FONT_HEIGHT, 90,
								0, 0, "simsun",
								des.substring(i * fontNum, (i + 1) * fontNum));
					} else {
						TSCLIB_DLL_GP3150TN.INSTANCE.windowsfont(x, y2, FONT_HEIGHT, 90,
								0, 0, "simsun", des.substring(i * fontNum));
					}
			...
			TSCLIB_DLL_GP3150TN.INSTANCE.windowsfont(x, y, FONT_HEIGHT, 90, 0, 0,
					"simsun", des);
		}
...
//		String des ="我试试走@￥sdfasdfsdsdfasfsdtes我试试走@￥sdfasdfsdsdfasfsdtest";
		...
		getPrintList();
		execute("000107020080001655,,,150500001A","2311040086875394", "23000039130", "01",
				"包装材料、标识-包装材料-胶纸","中国石油化工股份有限公司物资装备部中国石油化工股份有限公司物资装备部","","", "Microsoft XPS Document Writer",1);
		...
		if (StringUtils.isNotBlank(des)) {
			...
			for (int i = 0; i < charArray.length; i++) {
				...
				if (fn<0) {
					x = x + 40;
					TSCLIB_DLL_GP3150TN.INSTANCE.windowsfont(x, y, FONT_HEIGHT, 90, 0,
							0, "simsun", sb.substring(0, sb.length()-1));
	 ...
	 * @param index 
	 */
	public static void execute(String barCode,String mCode,String orderCode,String lineNo,String desc,String supplier,String fourPos,String speStore,String machineName, int index){
		System.setProperty("jna.encoding", "GBK");
		Print_Qrcode(barCode, mCode,
				orderCode, lineNo,
				desc,supplier,fourPos,speStore,machineName,index);
	}
}
